...
var _APP_PROJECT_CONFIG, _MODULE_PROJECT_CONFI;
...
function getNpmModulePrefs(args, cb) {
  ...
  if (args.f || args.force || 'umd' in args && 'es-modules' in args) {
    return process.nextTick(cb, null, { umd: umd, esModules: esModules });
  }
...
function initGit(args, cwd, cb) {
  ...
  if (_glob2.default.sync('.git/', { cwd: cwd }).length > 0) {
    return process.nextTick(cb);
  }

  (0, _child_process.exec)('git --version', { cwd: cwd, stdio: 'ignore' }, function (err) {
    if (err) return cb();
    var spinner = (0, _ora2.default)('Initing Git repo').start();
    (0, _runSeries2.default)([function (cb) {
      return (0, _child_process.exec)('git init', { cwd: cwd }, cb);
    }, function (cb) {
      return (0, _child_process.exec)('git add .', { cwd: cwd }, cb);
    }, function (cb) {
      return (0, _child_process.exec)('git commit -m "Initial commit from nwb v' + _package2.default.version + '"', { cwd: cwd }, cb);
    }], function (err) {
      if (err) {
        spinner.fail();
...
var APP_PROJECT_CONFIG = (_APP_PROJECT_CONFIG = {}, _APP_PROJECT_CONFIG[_constants.INFERNO_APP] = {
  dependencies: ['inferno', 'inferno-component', 'inferno-compat']
}, _APP_PROJECT_CONFIG[_constants.PREACT_APP] = {
  dependencies: ['preact', 'preact-compat']
}, _APP_PROJECT_CONFIG[_constants.REACT_APP] = {
  dependencies: ['react', 'react-dom']
}, _APP_PROJECT_CONFIG[_constants.WEB_APP] = {}, _APP_PROJECT_CONFIG);

var MODULE_PROJECT_CONFIG = (_MODULE_PROJECT_CONFI = {}, _MODULE_PROJECT_CONFI[_constants.REACT_COMPONENT] = {
  devDependencies: ['react', 'react-dom'],
  externals: { react: 'React' }
}, _MODULE_PROJECT_CONFI[_constants.WEB_MODULE] = {}, _MODULE_PROJECT_CONFI);
...
function createAppProject(args, projectType, name, targetDir, cb) {
  ...
  var templateVars = { name: name, nwbVersion: NWB_VERSION };
  (0, _runSeries2.default)([function (cb) {
...
function createModuleProject(args, projectType, name, targetDir, cb) {
  var _MODULE_PROJECT_CONFI2 = MODULE_PROJECT_CONFIG[projectType],
      ...
      devDependencies = _MODULE_PROJECT_CONFI3 === undefined ? [] : _MODULE_PROJECT_CONFI3,
      _MODULE_PROJECT_CONFI4 = _MODULE_PROJECT_CONFI2.externals,
      externals = _MODULE_PROJECT_CONFI4 === undefined ? {} : _MODULE_PROJECT_CONFI4;
  ...
  getNpmModulePrefs(args, function (err, prefs) {
    ...
    var nwbConfig = {
      ...
      npm: {
        esModules: esModules,
        umd: umd ? { global: umd, externals: externals } : false
      }
    };
